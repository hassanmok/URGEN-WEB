...
export function DoctorCaseSubmitPanel({ m }: Props) {
  ...
  function addFilesFromList(list: FileList | null) {
    ...
    if (rejected.length > 0) {
      setMsg({
        ok: false,
        text: m.fileTypeErr + (rejected.length ? ` (${rejected.join(', ')})` : ''),
      })
    } else {
      setMsg(null)
    }
    ...
    setMsg({
      ...
      text:
        (res.count ?? selectedTests.size) > 1
          ? m.submitOkBatch.replace('{n}', String(res.count ?? selectedTests.size))
          : m.submitOk,
    })
    setName1('')
    setName2('')
...
const caseStatusClass: Record<string, string> = {
  ...
  accepted: 'bg-emerald-100 text-emerald-900 ring-emerald-300/80',
...
function CaseCard({
  ...
  const genderLabel =
    row.gender === 'male' ? m.genderMale : row.gender === 'female' ? m.genderFemale : m.genderOther
      ...
      >
        ...
        {caseTests.length > 0 && (
          <p className="mt-1 text-xs text-slate-600">
            {m.testsInRequest}:{' '}
            {caseTests.map((t) => testTitleFor(t.test_slug, catalogTests, locale)).join(' · ')}
          </p>
        )}
